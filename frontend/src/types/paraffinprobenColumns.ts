export const paraffinprobenDataColumns = [
    { key: "patient_ID_intern", label: "Patienten ID (intern)" },
    { key: "sap_id", label: "SAP ID" },
    { key: "created_at", label: "Erstellungsdatum" },
    { key: "probenart", label: "Probenart" },
    { key: "probe_erhalten_zeit", label: "Probe erhalten (Uhrzeit)" },
    { key: "probengröße", label: "Probengröße" },
    { key: "bearbeiter_in", label: "Bearbeiter_in" },
    { key: "anmerkungen_probennahme", label: "Anmerkungen Probennahme" },
    { key: "anmerkungen_probenaufbereitung", label: "Anmerkungen Probenaufbereitung" },
    { key: "lagerraum", label: "Lagerraum" },
    { key: "anmerkungen", label: "Anmerkungen"}

];
